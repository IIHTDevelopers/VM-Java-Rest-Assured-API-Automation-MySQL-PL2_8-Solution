...
//				"Response must contain all required fields (name, id, status, jobtitle)");
		...
		List<Object> itemIds = customResponse.getIds();
		List<Object> itemTitles = customResponse.getTitles();

		Assert.assertNotNull(itemTitles, "Title list should not be null.");
		Assert.assertFalse(itemTitles.isEmpty(), "Title list should not be empty.");
	...
	public void GetLeaveWorkWeek() throws IOException {
		...
		System.out.println("Workweek Data: " + customResponse.getWorkweekData());
		...
		Assert.assertNotNull(customResponse.getWorkweekData(), "Workweek data must not be null");

		Assert.assertFalse(customResponse.getWorkweekData().isEmpty(), "Workweek data must not be empty");

		// Ensure all days 0-6 are present in the map
		for (int day = 0; day <= 6; day++) {
			Assert.assertTrue(customResponse.getWorkweekData().containsKey(String.valueOf(day)),
					"Workweek data must contain key for day: " + day);
			Assert.assertNotNull(customResponse.getWorkweekData().get(String.valueOf(day)),
...
public void DeleterecruitmentCad() throws Exception {
        ...
        Response response = RestAssured.given()
                ...
                .get("https://yakshahrm.makemylabs.in/orangehrm-5.7/web/index.php/api/v2/recruitment/candidates")
                .then()
                .statusCode(200)
                .extract()
                .response();
